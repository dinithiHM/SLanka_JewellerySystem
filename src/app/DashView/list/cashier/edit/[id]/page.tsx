...
const EditCashierPage = () => {
  ...
  useEffect(() => {
    const fetchCashier = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`http://localhost:3002/cashiers/${id}`);

        if (!response.ok) {
          throw new Error(`Failed to fetch cashier: ${response.status}`);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3002/cashiers/update/${id}`, {
        method: "PUT",
        headers: {
      ...
      router.push("/DashView/list/cashier");
    } catch (error) {
    ...
    return (
      <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
        <div className="p-4 bg-red-100 text-red-700 rounded-md">
          ...
          <p>{error}</p>
          <Link href="/DashView/list/cashier" className="mt-4 inline-block text-blue-600 hover:underline">
            Back to Cashiers
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center mb-6">
        <Link href="/DashView/list/cashier" className="mr-4">
          <button className="p-2 rounded-full bg-gray-100 hover:bg-gray-200">
            <ArrowLeft size={20} />
          </button>
        </Link>
        <h1 className="text-xl font-semibold">Edit Cashier</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end">
          <Link href="/DashView/list/cashier">
