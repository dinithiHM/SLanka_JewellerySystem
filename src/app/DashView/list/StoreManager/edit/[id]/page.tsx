...
const EditStoreManagerPage = () => {
  ...
  // Fetch store manager data
  useEffect(() => {
    const fetchStoreManager = async () => {
      setIsLoading(true);
      setError(null);
      try {
        const response = await fetch(`http://localhost:3002/store-managers/${id}`);

        if (!response.ok) {
          throw new Error(`Failed to fetch store manager: ${response.status}`);
        }
        ...
        if (data) {
          ...
          setRole(data.role || "Store Manager");
          setBranchId(data.branch_id || 1);
        } else {
          throw new Error("Store manager data not found");
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3002/store-managers/update/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to update store manager");
      }

      alert("Store Manager updated successfully!");
      router.push("/DashView/list/StoreManager");
    } catch (error) {
    ...
    return (
      <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
        <div className="p-4 bg-red-100 text-red-700 rounded-md">
          ...
          <p>{error}</p>
          <Link href="/DashView/list/StoreManager" className="mt-4 inline-block text-blue-600 hover:underline">
  ...
  return (
    <div className="bg-white p-6 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center mb-6">
        <Link href="/DashView/list/StoreManager" className="mr-4">
          <button className="p-2 rounded-full bg-gray-100 hover:bg-gray-200">
            <ArrowLeft size={20} />
          </button>
        </Link>
        <h1 className="text-xl font-semibold">Edit Store Manager</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end">
          <Link href="/DashView/list/StoreManager">
...
export default EditStoreManagerPage;
