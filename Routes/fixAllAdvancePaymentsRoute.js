...
// Route to fix all advance payments for custom orders
router.get("/fix-all-advance-payments", (req, res) => {
  console.log('GET /fix-all-advance-payments - Fixing all advance payments for custom orders');
  ...
  con.beginTransaction((transErr) => {
    ...
    // Step 1: Clear all existing advance payments for custom orders
    // This ensures we don't have duplicate entries
    const clearAdvancePaymentsSql = `
      DELETE FROM advance_payments
      WHERE is_custom_order = 1
    `;

    con.query(clearAdvancePaymentsSql, (clearErr) => {
      if (clearErr) {
        return con.rollback(() => {
          console.error("Error clearing existing advance payments:", clearErr);
          res.status(500).json({ message: "Database error", error: clearErr.message });
        });
      }

      console.log('Cleared existing advance payments for custom orders');

      // Step 2: Get all custom orders with their payments
      ...
      con.query(getAllOrdersSql, (getErr, allPayments) => {
        if (getErr) {
          return con.rollback(() => {
            console.error("Error getting all payments:", getErr);
        ...
        if (allPayments.length === 0) {
          return con.commit(() => {
            res.json({ message: "No payments found to process" });
          });
        }
        ...
        const processOrder = (orderIds, index) => {
          if (index >= orderIds.length) {
            ...
            return con.commit((commitErr) => {
              ...
              // Step 3: Update the advance_amount in custom_orders to match the sum of payments
              const updateAdvanceAmountsSql = `
              ...
              con.query(updateAdvanceAmountsSql, (updateErr) => {
                if (updateErr) {
                  console.error("Error updating advance amounts:", updateErr);
                  return res.status(500).json({ message: "Database error", error: updateErr.message });
                }

                // Step 4: Update the payment status based on the advance_amount vs estimated_amount
                ...
                con.query(updatePaymentStatusSql, (statusErr) => {
                  if (statusErr) {
                  ...
                  res.json({ 
                    message: "All advance payments fixed successfully", 
          ...
          con.query(sequenceQuery, [`${referencePrefix}%`], (seqErr, seqResults) => {
            ...
            const processPayment = (paymentIndex) => {
              ...
              // Calculate running total of payments for this order up to this payment
              let runningTotal = 0;
              for (let i = 0; i <= paymentIndex; i++) {
                runningTotal += parseFloat(payments[i].payment_amount);
              }
              
              // Calculate balance amount based on estimated amount and running total
              const balance_amount = parseFloat(payment.estimated_amount) - runningTotal;
              ...
              con.query(advancePaymentSql, [
                ...
                payment.notes || 'Advance payment for custom order',
                payment.created_by,
                payment.branch_id,
...
// Route to update balance amounts in real-time
router.get("/update-balance-amounts", (req, res) => {
  console.log('GET /update-balance-amounts - Updating balance amounts for all custom orders');
  ...
  con.query(updateAdvanceAmountsSql, (updateErr) => {
    ...
    con.query(updatePaymentStatusSql, (statusErr) => {
      ...
      res.json({ 
        message: "Balance amounts updated successfully"
      });
    });
  });
});

export { router as fixAllAdvancePaymentsRouter };
